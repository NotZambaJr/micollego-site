...
export default function Model({mouse}) {
  ...
  const { nodes } = useGLTF("/assets/gltf/floating_shapes4.glb");
  return (
    <group>
...
function Mesh({node, multiplier, mouse, isActive}) {
  ...
  const material = new THREE.MeshStandardMaterial({color: 0x153dbc, roughness: 1, metalness: 0, side: THREE.DoubleSide});
